...
#   f_agent_contact_fila_diario.csv
#   f_css_detalhado.csv
#   f_css_atendente.csv
...
COLUNAS_TEXTO_SAIDA = {
    "Data", "Periodo_Inicio", "Periodo_Fim", "Ano_Mes", "Mes_Referencia", "Pasta_Origem",
...
def tratar_agent_contact_diario(pasta_entrada: Path, pasta_saida: Path) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    salvar(diario, pasta_saida, "f_agent_contact_diario.csv")
    salvar(fila, pasta_saida, "f_agent_contact_fila_diario.csv")
    return diario, fila
...
def tratar_css(pasta_entrada: Path, pasta_saida: Path) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    resumo = (
        validas.groupby(
            ["Data", "Periodo_Inicio", "Periodo_Fim", "Ano_Mes", "Tem_Data_Diaria", "Atendente_ID", "Atendente"],
            dropna=False,
            as_index=False
        )
        .agg(
    ...
    periodo_atendente = (
        validas.groupby(
            ["Periodo_Inicio", "Periodo_Fim", "Ano_Mes", "Atendente_ID", "Atendente"],
            dropna=False,
            as_index=False
        )
        .agg(
    ...
    periodo_geral = (
        validas.groupby(
            ["Periodo_Inicio", "Periodo_Fim", "Ano_Mes"],
            dropna=False,
            as_index=False
        )
        .agg(
...
def tratar_css_fila_diario(pasta_entrada: Path, pasta_saida: Path) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    fila = (
        validas.groupby(["Data", "Fila"], as_index=False)
        .agg(
            Total_Respostas_CSS=("Quantidade", "sum"),
            Qtd_Positiva=("Qtd_Positiva", "sum"),
            Qtd_Neutra=("Qtd_Neutra", "sum"),
...
def criar_indicadores_agentes(
    ...
    if css_atendente is not None and not css_atendente.empty:
        ...
        periodo = (
            c.groupby(["Atendente_ID", "Atendente"], dropna=False, as_index=False)
            .agg(
...
def processar_lote_mensal(pasta_mes: Path, pasta_temp: Path) -> Dict[str, pd.DataFrame]:
    ...
    return {
        ...
        "f_agent_contact_diario": adicionar_origem_lote(agent, pasta_mes),
        "f_agent_contact_fila_diario": adicionar_origem_lote(agent_fila, pasta_mes),
...
CHAVES_INCREMENTAIS = {
    ...
    "f_css_detalhado": ["Periodo_Inicio", "Periodo_Fim", "Atendente_ID", "Script", "Pergunta", "Resposta"],
    ...
    "f_indicadores_agentes_periodo": ["Atendente_ID"],
    "f_indicadores_gerais": ["Data"],
    "f_indicadores_gerais_periodo": ["Periodo_Inicio", "Periodo_Fim"],
}

COLUNAS_DATA_CHAVE = {"Data", "Periodo_Inicio", "Periodo_Fim"}
...
def gerar_saida_historica(
    ...
    ordenacoes = {
        ...
        "f_agent_contact_diario": ["Data", "Atendente", "Pasta_Origem"],
        "f_agent_contact_fila_diario": ["Data", "Atendente", "Fila", "Pasta_Origem"],
    ...
    dim = criar_dim_atendentes_consolidada(
        historico.get("f_agent_contact_diario", pd.DataFrame()),
...
def main():
    parser = argparse.ArgumentParser(description="ETL Contact Center histórico incremental para modelo Power BI.")
    parser.add_argument(
        "--base-dir",
        default=r"C:\Users\GustavoCardoso\Downloads\RPA_SSRS",
        help="Pasta base com subpastas entrada e saida. Ex.: C:\\RPA_SSRS",
    )
    parser.add_argument(
        "--entrada",
        default=None,
        help="Pasta raiz onde estão os relatórios ou as subpastas mensais. Ex.: C:\\RPA_SSRS\\entrada",
    )
    parser.add_argument(
        "--saida",
        default=None,
        help="Pasta onde serão salvos os CSVs finais consolidados. Ex.: C:\\RPA_SSRS\\saida",
    )
    parser.add_argument(
        "--reprocessar-tudo",
        action="store_true",
        help="Recria os arquivos finais do zero. Sem esse parâmetro, o script mantém a saída antiga e adiciona apenas datas/chaves novas.",
    )
    parser.add_argument(
        "--substituir-datas-existentes",
        action="store_true",
        help="Atualiza registros de datas/chaves já existentes com os valores recém-processados. Use apenas quando quiser corrigir meses já carregados.",
    )
    args = parser.parse_args()

    base_dir = Path(args.base_dir)
    pasta_entrada = Path(args.entrada) if args.entrada else base_dir / "entrada"
    pasta_saida = Path(args.saida) if args.saida else base_dir / "saida"
    pasta_saida.mkdir(parents=True, exist_ok=True)

    print("Iniciando ETL Contact Center histórico...")
    print(f"Entrada raiz: {pasta_entrada}")
    ...
    gerar_saida_historica(
        pastas,
        pasta_saida,
        incremental=not args.reprocessar_tudo,
        substituir_chaves_existentes=args.substituir_datas_existentes,
    )

    print("\nConcluído. Os arquivos finais estão na pasta de saída, mantendo o histórico antigo quando o modo incremental estiver ativo.")
